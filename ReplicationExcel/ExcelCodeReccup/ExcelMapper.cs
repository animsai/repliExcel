...
namespace ReplicationExcel
{
    public class ExcelMapper
    {
        ...
        public void Read2(Student destination, string excelFileName) // version SEAN METRY
        {
            destination.Numbers = new List<string>();
            using (ExcelManager em = new ExcelManager())
            {
                em.Open(excelFileName);
                ...
                foreach (PropertyInfo propInfo in objType.GetProperties())
                {
                    ...
                    foreach (Attribute att in attributes)
                    {
                        ...
                        if (fromRange != null)
                        {
                            if (fromRange.StartCellAddress == "A3")
                            {
                                ...
                                {
                                    destination.Numbers.Add((string)Number);
                                    
                                }
                               
                                
                            }
                            if (fromRange.StartCellAddress == "D3")
                            {
                                ArrayList values = em.GetRangeValues(fromRange.StartCellAddress, fromRange.EndCellAddress, fromRange.Category);
                                ...
                                foreach (var Number in values)
                                {
                                    destination.Name.Add((string)Number);

                                }
                            }
                            if (fromRange.StartCellAddress == "C3")
                            {
                                ArrayList values = em.GetRangeValues(fromRange.StartCellAddress, fromRange.EndCellAddress, fromRange.Category);
                                ...
                                foreach (var Number in values)
                                {
                                    destination.Family.Add((string)Number);
